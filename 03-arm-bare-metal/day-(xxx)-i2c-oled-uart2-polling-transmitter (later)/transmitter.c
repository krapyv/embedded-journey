...
void oled_init()
{
    ...
    GPIOB_MODER &= ~((0x3 << 12) | (0x3 << 14)); // clearning bits
    GPIOB_MODER |= ((0x2 << 12) | (0x2 << 14));  // set to 10

    GPIOB_AFRL &= ~((0xF << 24) | (0xF << 28)); // clear bits
    GPIOB_AFRL |= ((0x4 << 24) | (0x4 << 28));

    // configure I2C1
    // 100kHz standard mode, 16MHz APB1 clock
    I2C1_CR2 = 16;
    I2C1_CCR = 80;
    I2C1_TRISE = 17;

    // enable 12C1
    I2C1_CR1 |= (1 << 0); // set bit 0 PE to 1
...
void usart2_init()
{
    ...
    // enable USART by setting the bit 13 of the Control Register 1 to 1
    USART2_CR1 |= (1 << 13);

    // enable Transmitter (to be able to send data from the Blackpill to the computer) by setting the bit 3 of CR1 to 1
    USART2_CR1 |= (1 << 3);
}
